...
public class PredicateInSelenium
{
	...
	public void test()
	{
		...
		//Removing the blank texts
		Predicate<WebElement> isBlank = x -> x.getText().trim().length() == 0;
		elementList.removeIf(isBlank);
		
		
		//Removing text which doesn't contains "s"
		Predicate<WebElement> notContainsS = x -> x.getText().trim().toLowerCase().contains("s");
		elementList.removeIf(notContainsS);
	}
